...
echo "pwd" > credentials/password
echo $CW_OP_SEQUENCER_KEY > credentials/block-signer-key
echo $CW_OP_P2P_PRIVATE_KEY_NODE1 > credentials/opnode_p2p_priv.txt
...
sudo docker run -it -v ./credentials:/credentials -v ./datadir:/datadir --name op_geth_account_importing $CW_OP_IMAGE_OP_GETH geth account import --datadir=/datadir --password=/credentials/password /credentials/block-signer-key
...
sudo docker rm op_geth_account_importing
